...
class DGSMP(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        ## Dense connection
        self.conv = nn.Conv2d(self.Ch, self.embed_dim, kernel_size=3, stride=1, padding=1)
        ...
        self.conv_U = nn.Conv2d(self.embed_dim, 3, 3, 1, 1)
        self.conv_W = nn.Sequential(nn.Conv2d(self.embed_dim, 3, 3, 1, 1))
    ...
    def forward_features(self, fufeat):
        ...
        for layer in self.layers:
            ...
            if len(Feat_list) == 3:
                # print(Feat_list[1].size(), x_feat.size())
    ...
    def forward(self, y):
        h, w = y.shape[2:]
        Xt = torch.nn.functional.interpolate(y, scale_factor=self.factor, mode='bilinear', align_corners=False)

        self.mean = self.mean.type_as(Xt)
        Xt = (Xt - self.mean) * self.img_range
        ...
        for i in range(0, self.s):
            ...
            U = U[:, :, :h * self.factor, :w * self.factor]
            W = W[:, :, :h * self.factor, :w * self.factor]
        ...
        Xt = Xt / self.img_range + self.mean
        return Xt
...
if __name__ == '__main__':
    ...
    model = DGSMP(upscale=1, img_size=(height, width),
                  ...
                  embed_dim=64, num_heads=[4, 4, 4, 4, 4], mlp_ratio=2)
