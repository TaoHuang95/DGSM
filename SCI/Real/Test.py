...
for i in range(55,151,10):  # number of model.pth
    ...
    for j in range(5):
        ...
        k = k + 1
        model_dir = '/home/huangtao/Code/Spectral_SCI/Transformer/Real_PAMI/Result1/' + str(i)
        if not os.path.isdir(model_dir):  # Create the model directory if it doesn't exist
            os.makedirs(model_dir)
        res = result.cpu().permute(2,3,1,0).squeeze(3).numpy()
        save_path = '/home/huangtao/Code/Spectral_SCI/Transformer/Real_PAMI/Result1/' + str(i) + '/' + str(j + 1) + '.mat'
